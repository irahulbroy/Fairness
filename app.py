...
with cols[0]:
    ...
    ax1.axvline(alpha_grid[optimal_idx], color='red', linestyle='--', label=f'Optimal α: {alpha_grid[optimal_idx]:.2f}')
    ax1.legend()
    st.pyplot(fig1)
    buf1 = io.BytesIO()
...
with cols[1]:
    ...
    ax2.grid(True)
    optimal_idx = np.nanargmax(V_vals)
    ax2.axvline(alpha_grid[optimal_idx], color='red', linestyle='--', label=f'Max V(α)')
    ax2.legend()
    st.pyplot(fig2)
    buf2 = io.BytesIO()
...
with cols2[0]:
    ...
    ax3.axvline(alpha_grid[optimal_surplus_idx], color='black', linestyle='--', label=f'Surplus Optimal α: {alpha_grid[optimal_surplus_idx]:.2f}')
    ax3.legend()
    st.pyplot(fig3)
    buf3 = io.BytesIO()
...
with cols2[1]:
    ...
    ax4.axvline(alpha_grid[optimal_idx], color='red', linestyle='--', label=f'Platform Optimal α: {alpha_grid[optimal_idx]:.2f}')
    ax4.axvline(alpha_grid[optimal_earning_idx], color='black', linestyle='--', label=f'Total Provider Optimal α: {alpha_grid[optimal_earning_idx]:.2f}')
    ax4.legend()
    st.pyplot(fig4)
    buf4 = io.BytesIO()
